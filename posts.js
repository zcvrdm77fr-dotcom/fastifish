...
router.get('/:id/comments', (req, res) => {
  ...
  `).all(post.id);
  res.json({
    comments: rows.map(r => ({ id: r.id, username: r.username, body: r.body, createdAt: r.created_at }))
  });
});
...
router.post('/:id/comments', requireAuth, (req, res) => {
  ...
  if (body.length > 1000) {
    return res.status(400).json({
      error: 'Kommentti on liian pitkä.'
    });
  }
